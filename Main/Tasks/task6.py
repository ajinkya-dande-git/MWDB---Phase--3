...
def startTask6():
    ...
    numberOfRelavant = int(input("Number of relavant images "))
    numberOfIrRelavant = int(input("Number of irrelavant images "))
    relavantImages = []
    irrelavantImages = []
    for i in range(numberOfRelavant):
        relavantImages.append(input("Please " + str(i + 1) + " relavant image "))
    for i in range(numberOfIrRelavant):
        irrelavantImages.append(input("Please " + str(i + 1) + " irrelavant image "))
    if feedbackSystem == 1:
        ...
        for row in range(steady_state.shape[0]):
            news = ""
            news = news + "<img src='"
            news = news + join(config.FULL_IMAGESET_FOLDER, row)
            news = news + "'>"
            s = s + news
