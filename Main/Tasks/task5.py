...
def index_imgs():
    ...
    global hash_rep
    database = []
    json_files = os.listdir(config.FEATURE_DESCRIPTOR_FOLDER)
    ...
    for i in range(len(hash_rep)):
        ...
        for img in json_files:
            with open(path.join(config.FEATURE_DESCRIPTOR_FOLDER, img + '.json'), 'r', encoding='utf-8') as f:
...
def lsh_index_structure(l, k):
    global hash_rep
    global database
    with open(path.join(config.FEATURE_DESCRIPTOR_FOLDER, 'Hand_0000002.jpg.json'), 'r', encoding='utf-8') as f:
        data = json.load(f)
...
def store_results_as_json(sorted_imgs, l, k, t, query_img):
    ...
    results_dict = dict()
    for img in sorted_imgs[:t]:
        with open(path.join(config.FEATURE_DESCRIPTOR_FOLDER, img[0] + '.json'), 'r', encoding='utf-8') as f:
...
def startTask5():
    ...
    with open(path.join(config.FEATURE_DESCRIPTOR_FOLDER, query_img + '.json'), 'r', encoding='utf-8') as f:
